...
def amp2d(y,forward_fun,inverse_fun,sigmaType=2,randshift=False,tol=1e-8,x=None,ignore_residual=False,disp=False,maxiter=100):
    ...
    if disp:
        ...
        if disp:
            table = Table([ 'iter','resid','resid diff','MSE' ],[ len(repr(maxiter)),8,8,8 ],[ 'd','e','e','e' ])
            hdr = table.header()
    ...
    OptimumLambdaSigned = loadmat(dirname(__file__) + '/OptimumLambdaSigned.mat') # this file has the optimal values of lambda
    ...
    for abc in range(int(maxiter)):
        ...
        # Soft Thresholding
        wn1 = (np.abs(temp_z) > lambdas*sigma_hat)*(np.abs(temp_z) - lambdas*sigma_hat)*np.sign(temp_z)
        ...
        if disp:
            logging.info(table.row([ abc,nn[abc+1],res_diff[abc],compare_mse(xabs,np.abs(wn1)) ]))
