...
class GSFMTestCase(unittest.TestCase):
    ...
    def test_simulated_phantom_2d(self):
        ...
        m1 = gre_sim(T1s,T2s,TR=args['TR'],TE=TE1,alpha=args['alpha'],field_map=args['field_map'],dphi=0,M0=PD,tol=1e-4)
        m2 = gre_sim(T1s,T2s,TR=args['TR'],TE=TE2,alpha=args['alpha'],field_map=args['field_map'],dphi=0,M0=PD,tol=1e-4)
