...
def dTV(A,eps=1e-8):
    ...
    den1 = np.sqrt(np.abs(A - np.roll(A,-1,axis=0))**2 + np.abs(A - np.roll(A,-1,axis=1))**2) + eps

    num2 = np.roll(A,1,axis=0) - A
    den2 = np.sqrt(np.abs(np.roll(A,1,axis=0) - A)**2 + np.abs(np.roll(A,1,axis=0) - np.roll(A,(1,-1)))**2) + eps

    num3 = np.roll(A,1,axis=1) - A
    den3 = np.sqrt(np.abs(np.roll(A,1,axis=1) - A)**2 + np.abs(np.roll(A,1,axis=1) - np.roll(A,(-1,1)))**2) + eps
