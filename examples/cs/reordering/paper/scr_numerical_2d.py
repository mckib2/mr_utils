...
if __name__ == '__main__':
    ...
    recon_none = proximal_GD(
        ...
        unsparsify, reorder_fun=None, mode='soft', alpha=alpha0,
    ...
    if 'monosort' in run:
        ...
        recon_sort2d = proximal_GD(
            kspace_u, uft.forward_ortho, uft.inverse_ortho, sparsify,
            unsparsify, reorder_fun=monosort, mode='soft',
            alpha=alpha0, thresh_sep=True, selective=None, x=x,
            ignore_residual=False, disp=True, maxiter=500)
        view(recon_sort2d)

        # Find k for recon_sort2d
        idx_r = np.unravel_index(
            idx_ro.real.astype(int), recon_sort2d.shape)
        idx_i = np.unravel_index(
            idx_ro.imag.astype(int), recon_sort2d.shape)
        coeffs = sparsify((
            recon_sort2d.real[
                idx_r] + 1j*recon_sort2d.imag[idx_i]).reshape(
    ...
    if 'lagrangian' in run:
        ...
        recon_ls = proximal_GD(
            kspace_u, uft.forward_ortho, uft.inverse_ortho, sparsify,
            unsparsify, reorder_fun=lagrangesort, mode='soft',
            alpha=alpha0, thresh_sep=True, selective=None, x=x,
            ignore_residual=False, disp=True, maxiter=500)
        np.save(path + 'recon_ls.npy', recon_ls)
        view(recon_ls)
